#!/usr/bin/env python3
"""Generate the emoji search-name table (item 4 fix).
...
def main() -> int:
    ...
    for chunk in re.findall(r'"([^"]+)"\s*\}', text):
        for glyph in chunk.split(" "):
            ...
            if glyph and not glyph.isascii() and glyph not in glyphs:
                glyphs.append(glyph)
    if not glyphs:
        print("could not read any glyph from picker_data.vala", file=sys.stderr)
        return 1
    ...
    print(f"{len(glyphs)} glyphs -> {TARGET.relative_to(REPO)}")
    return 0
