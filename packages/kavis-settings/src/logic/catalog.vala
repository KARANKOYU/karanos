...
namespace Kavis.Settings.Catalog {
    ...
    public Sub[] subs () {
        return {
            ...
            { "security",   "status",      _("Status") },
            { "security",   "clipboard",   _("Clipboard privacy") },
            { "security",   "scan",        _("Malware scan") },
    ...
    public Item[] items () {
        return {
            ...
            { "security", "status", _("Kernel hardening"),
              _("What the running kernel actually has switched on") + " security hardening kernel sysctl kptr dmesg ptrace protection" },
            { "security", "status", _("Secure Boot"),
              _("Whether the firmware is verifying what boots") + " secure boot uefi firmware signature" },
            { "security", "status", _("Firewall"),
              _("Whether anything is filtering connections") + " firewall nftables ufw ports network security" },
            { "security", "clipboard", _("Watch clipboard reads"),
              _("Which application reads the clipboard") + " clipboard privacy security read paste who spy monitor" },
            { "security", "scan", _("Malware scan"),
              _("Scans the downloads folder with ClamAV") + " antivirus malware virus scan clamav security downloads" },
            { "system", "about", _("About"),
