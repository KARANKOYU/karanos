...
Metinler docs/karanos-arayuz-metinleri.md `panel.*` anahtarlarından.
"""
...
.karanos-guc-kutu.duz {
  border-radius: 0;
  box-shadow: none;
}
.karanos-guc-kutu button {
...
.karanos-guc-kutu button:hover {
  background-color: #1D2C38;
}
.karanos-guc-kutu button:active {
...
class GucMenusu(Gtk.Window):
    def __init__(self):
        ...
        # varken görünüyor. Karan OS'ta picom autostart'tan başlıyor;
    ...
    def _kur(self):
        ...
        self.kutu.get_style_context().add_class("karanos-guc-kutu")
        if not self.bilesik:
