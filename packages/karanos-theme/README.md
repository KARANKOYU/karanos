...
Karan OS'un görünümü: GTK teması, simgeler, imleçler, duvar kağıtları ve
dağıtım kimliği.
...
| Yol | İçerik |
|---|---|
| `/usr/share/themes/Karan/gtk-3.0/` | `gtk.css` (açık), `gtk-dark.css` (koyu) |
| `/usr/share/themes/Karan/gtk-2.0/gtkrc` | eski GTK2 uygulamaları için renk uyumu |
| `/usr/share/themes/Karan/openbox-3/themerc` | pencere çerçevesi ve başlık çubuğu |
| `/usr/share/icons/Karan/` | K logosundan türetilen simgeler, Adwaita'yı miras alır |
| `/usr/share/icons/Karan-Cursors/` | 16 şekil, 106 ad, ikisi animasyonlu |
| `/usr/share/backgrounds/karanos/` | `karan`, `karan-koyu`, `karan-duz` (SVG + PNG + önizleme) |
| `/etc/gtk-3.0/settings.ini` | sistem geneli varsayılan tema/imleç/yazı tipi |
| `/etc/os-release` | `PRETTY_NAME="Karan OS 1.0"` |
...
```bash
tools/build-packages.sh karanos-theme     # depo kökünden
```

Çıktı `out/packages/karanos-theme_1.0_all.deb`. Script `assets/logo/`
